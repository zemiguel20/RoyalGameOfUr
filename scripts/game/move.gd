...
func is_capture():
	return to._pieces != null and to._pieces.front().player != _player 
...
func _calculate_spot_danger(spot: Spot, base_danger_score: float):
	...
	if (spot.is_safe or gives_extra_roll() or spot.is_player_exclusive(spot)):
		return 0
	...
	var opponent_id = General.get_other_player_id(_player)
