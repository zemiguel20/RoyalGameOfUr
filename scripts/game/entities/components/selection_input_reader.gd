...
##
## Distinguishes between clicking and holding by counting the time between
## a press and a release, and comparing it with a threshold.
...
@export_range(0.0, 1.0, 0.025, "suffix:s") var _hold_threshold = 0.3 
...
func _process(_delta: float) -> void:
	...
	if _is_pressing and Input.is_action_pressed("select"):
		...
		if elapsed_time_sec >= _hold_threshold and not _is_holding:
			_is_holding = true
			hold_started.emit()
...
## Activates or deactivates the ability to read input.
